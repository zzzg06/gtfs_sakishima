...
import { Train, Minus, Plus, RotateCcw, List, X } from "lucide-react"

// 管理画面用の路線図（運行状況マップ）。在線表示(/live)は1線ずつ縦に並べるのに対し、
// こちらは路線網全体を1枚の模式図に描き、列車を線上に配置する。ひとまず列車のみ（バスは対象外）。
// 位置はダイヤ＋遅延から実時間で算出（/live のダイヤ予測と同じロジック）。
...
export function RouteStatusMap() {
  ...
  // 図の幅をビューポート幅に合わせる（横スクロールなしで全体が入る倍率を基準にする）
  const measure = useCallback(() => {
    const el = viewportRef.current
    if (!el) return
    const w = el.clientWidth
    if (w > 0) setFit(Math.min(1, w / MAP_VIEWBOX.width))
  }, [])
  useEffect(() => {
    measure()
    const el = viewportRef.current
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            路線図上に列車の在線位置を表示します（ダイヤに基づく推定・{REFRESH_MS / 1000}秒ごとに自動更新）。
          </p>
        </div>
          ...
          <div className="w-full space-y-3 lg:w-56 lg:flex-shrink-0">
            ...
            <div className="rounded-lg border border-border bg-card p-3 text-sm">
              ...
              <p className="mt-2 text-xs text-muted-foreground">
                列車は進行方向に向かって左側に表示します。アイコンをクリックすると運用の詳細が出ます。
              </p>
            </div>
          </div>
        )}
